...
int
ip_selector_match_validate_selector(struct IPSelector *selector,
				    uint32_t length)
{
	...
	if (length < size) {
		DEBUG_FAIL(check,
			   ...
			   selector, length, (int)sizeof *selector);

		return -1;
	}

	size +=
	    selector->source_endpoint_count * sizeof(struct IPSelectorEndpoint);
	...
	size +=
	    selector->source_address_count * sizeof(struct IPSelectorAddress);

	size +=
	    selector->destination_address_count *
	    sizeof(struct IPSelectorAddress);

	size += selector->source_port_count * sizeof(struct IPSelectorPort);
	...
	if (length < size) {
		DEBUG_FAIL(check,
			   "Selector %p check failed length %d shorter "
			   "than computed size %d.", selector, length, size);
	...
	if (selector->bytecount != size) {
		DEBUG_FAIL(check,
			   ...
			   selector, selector->bytecount, size);
					  ...
					  *selector_group, int bytecount)
{
	if (bytecount < sizeof *selector_group) {
		DEBUG_FAIL(check,
			   ...
			   (int)sizeof *selector_group, (int)bytecount);
	...
	if (bytecount != selector_group->bytecount) {
		DEBUG_FAIL(check,
			   ...
			   (int)selector_group->bytecount, (int)bytecount);
...

	{
		...
		for (i = 0; i < selector_group->selector_count; i++) {
			status =
			    ip_selector_match_validate_selector((struct
								 IPSelector
								 *)(start +
								    offset),
								bytecount -
		...
		if (offset != bytecount) {
			DEBUG_FAIL(check,
				   ...
				   selector_group, offset, bytecount);
...
static int
ip_selector_match_debug_log_endpoint_list(const uint8_t * base,
					  int offset, int endpoint_count)
{
	int i;
	...
	for (i = 0; i < endpoint_count; i++) {
		...
		DEBUG_LOW(dump,
			  ...
			  endpoint->port.begin, endpoint->port.end);
...
static int
ip_selector_match_debug_log_address_list(const uint8_t * base,
					 int offset, int address_count)
{
	int i;
	...
	for (i = 0; i < address_count; i++) {
		...
		DEBUG_LOW(dump,
			  ...
			  base, address->begin, address->end);
...
static int
ip_selector_match_debug_log_port_list(const uint8_t * base,
				      int offset, int port_count)
{
	int i;
...
void ip_selector_match_debug_log_selector(const struct IPSelector *selector)
{
	...
	DEBUG_LOW(dump,
		  ...
		  selector, selector->ip_version);

	DEBUG_LOW(dump,
		  "Selector %p: ip protocol %d.",
		  selector, selector->ip_protocol);

	DEBUG_LOW(dump,
		  "Selector %p: Source endpoint count %d.",
		  selector, selector->source_endpoint_count);

	offset =
	    ip_selector_match_debug_log_endpoint_list(base,
						      offset,
						      selector->
	...
	DEBUG_LOW(dump,
		  ...
		  selector, selector->destination_endpoint_count);

	offset =
	    ip_selector_match_debug_log_endpoint_list(base,
						      offset,
						      selector->
						      destination_endpoint_count);

	DEBUG_LOW(dump,
		  "Selector %p: Source address count %d.",
		  selector, selector->source_address_count);

	offset =
	    ip_selector_match_debug_log_address_list(base,
						     offset,
						     selector->
	...
	DEBUG_LOW(dump,
		  ...
		  selector, selector->destination_address_count);
	...
	DEBUG_LOW(dump,
		  ...
		  selector, selector->source_port_count);

	offset =
	    ip_selector_match_debug_log_port_list(base,
						  offset,
						  selector->source_port_count);

	DEBUG_LOW(dump,
		  "Selector %p: Destination port count %d.",
		  selector, selector->destination_port_count);
...
ip_selector_match_debug_log_group(const struct IPSelectorGroup *selector_group)
{
	DEBUG_LOW(dump,
		  ...
		  selector_group,
		  selector_group->selector_count, selector_group->bytecount);
...
const char *debug_str_ip_selector_fields(void *buf, const void *data, int len)
{
	...
	offset +=
	    snprintf(p + offset,
		     p_len,
		     "IPv%d proto %d [",
		     fields->ip_version, fields->ip_protocol);
	...
	if (fields->ip_protocol == /*      */ 1 ||
		...
		offset +=
		    snprintf(p + offset,
			     p_len - offset, "]:%d -> [", fields->source_port);
	}
	...
	if (fields->destination_port < 0) {
		...
		offset +=
		    snprintf(p + offset,
			     p_len - offset, "]:%d", fields->destination_port);
	}
...
static int
debug_dump_endpoint_list(void *context,
			 const char *prefix,
			 const uint8_t * base, int offset, int endpoint_count)
{
	int i;
	...
	for (i = 0; i < endpoint_count; i++) {
		...
		DEBUG_DUMP_LINE(context,
				"  %sendpoint ipv %d, proto %d, [%s - %s], [%d - %d].",
				prefix,
				endpoint->ip_version,
				endpoint->ip_protocol,
				debug_strbuf_ipaddress(DEBUG_STRBUF_GET(),
						       endpoint->address.begin),
				debug_strbuf_ipaddress(DEBUG_STRBUF_GET(),
						       endpoint->address.end),
				endpoint->port.begin, endpoint->port.end);
...
static int
debug_dump_address_list(void *context,
			const char *prefix,
			const uint8_t * base, int offset, int address_count)
{
	int i;
...
static int
debug_dump_port_list(void *context,
		     const char *prefix,
		     const uint8_t * base, int offset, int port_count)
{
	int i;
...
void debug_dump_ip_selector(void *context, const void *data, unsigned bytecount)
{
	...
	offset =
	    debug_dump_endpoint_list(context,
				     "src ",
				     base,
				     offset, selector->source_endpoint_count);
	...
	offset =
	    debug_dump_address_list(context,
				    "src ",
				    base,
				    offset, selector->source_address_count);
	...
	offset =
	    debug_dump_port_list(context,
				 "src ",
				 base, offset, selector->source_port_count);

	(void)
	    debug_dump_port_list(context,
				 "dst ",
				 base,
				 offset, selector->destination_port_count);
}
			     ...
			     const void *data, unsigned bytecount)
{
...

	{
		...
		for (i = 0; i < selector_group->selector_count; i++) {
			...
			debug_dump_ip_selector(context, selector,
					       selector->bytecount);
...
void ip_selector_match_debug_log_fields(const struct IPSelectorFields *fields)
{
	DEBUG_LOW(dump,
		  ...
		  fields->source_address,
		  fields->source_port,
		  fields->destination_address, fields->destination_port);
}

static int
ip_selector_match_port_value(const struct IPSelectorPort *port_selector,
			     int port)
{
...
static int
ip_selector_match_endpoint_match(const uint8_t * base,
				 int *offset,
				 int endpoint_count,
				 int ip_version,
				 int ip_protocol,
				 const uint8_t address[16], int port)
{
	...
	*offset += endpoint_count * sizeof(struct IPSelectorEndpoint);
	...
	for (index = 0; index < endpoint_count; index++) {
		const struct IPSelectorEndpoint *endpoint_selector =
		    endpoints + index;
		...
		if (ip_selector_match_port_value(&endpoint_selector->port, port)
		    == 0) {
			continue;
		}
...
static int
ip_selector_match_address_match(const uint8_t * base,
				int *offset,
				int address_count, const uint8_t address[16])
{
	...
	*offset += address_count * sizeof(struct IPSelectorAddress);
	...
	for (index = 0; index < address_count; index++) {
		const struct IPSelectorAddress *address_selector =
		    addresses + index;
...
static int
ip_selector_match_port_match(const uint8_t * base,
			     int *offset, int port_count, int32_t port)
{
...
static int
ip_selector_match_selector_match(const struct IPSelector *selector,
				 const struct IPSelectorFields *fields)
{
	...
	DEBUG_LOW(lookup, "Fields %p: matching selector %p.", fields, selector);
...

	{
		if (selector->ip_version != 0 &&
			...
			DEBUG_LOW(lookup,
				  "Fields %p: No match for protocol.", fields);
	...
	if (match) {
		if (!ip_selector_match_endpoint_match(base,
						      &offset,
						      selector->
						      source_endpoint_count,
						      fields->ip_version,
	...
	if (match) {
		if (!ip_selector_match_address_match(base,
						     &offset,
						     selector->
						     source_address_count,
						     fields->source_address)) {
	...
	if (match) {
		if (!ip_selector_match_port_match(base,
						  &offset,
						  selector->source_port_count,
						  fields->source_port)) {
			DEBUG_LOW(lookup,
				  ...
				  const struct IPSelectorFields *fields)
{
	...
	for (i = 0; i < selector_group->selector_count; i++) {
		...
		if (ip_selector_match_selector_match(selector, fields)) {
			match = true;
			DEBUG_LOW(lookup, "Fields %p on selector %p: match.",
				  fields, selector);
			break;
		} else {
			DEBUG_LOW(lookup, "Fields %p on selector %p: no match.",
				  fields, selector);
		}
